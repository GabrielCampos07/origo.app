...
export default function InvitePage() {
  ...
  useEffect(() => {
    async function checkInvite() {
      // P0 SEC (FRONTEND SECURITY CHECKER): GET /api/v1/invites/:token
      // - Token is opaque hashed single-use with TTL
      // - Backend returns one of 6 states (no internal IDs leaked)
      // - Generic error messages (no enumeration)
      const result = await apiGet<{
        valid: boolean;
        state: "valid" | "expired" | "used" | "email_exists";
      ...
      if (result.ok) {
        ...
        } else if (result.data.state === "email_exists") {
          setInviteState({ state: "email_exists" });
        } else {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    // P0 SEC (FRONTEND SECURITY CHECKER): POST /api/v1/auth/register/student
    // - Token is opaque hashed single-use with TTL
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-4 py-8">
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-sm">
        ...
        ) : inviteState.state === "email_exists" ? (
          <div className="text-center">
            ...
            <h2 className="mb-2 text-2xl font-semibold text-slate-900">E-mail já cadastrado</h2>
            <p className="text-slate-600">
