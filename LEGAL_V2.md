...
**Status:** ⚠️ **BLOCKED ON DB TEAM** — API code ready, waiting for `LegalAcceptance` Prisma model.

**Owner mandate:** 2026-09-13  
**API implementation:** Ready for testing once DB model lands
...
- `privacy_v2_2026-09-13` — Privacy Policy V2
- `terms_app_v2_2026-09-13` — App Terms of Service V2

### PROFESSIONAL Users (Checkout Gate)
Users with PROFESSIONAL role or during checkout MUST also accept:

- `terms_saas_v2_2026-09-13` — SaaS Terms V2
- `payments_notice_v2_2026-09-13` — Payments Notice V2

### Optional / UI-Only Documents
The following are **NOT** enforced at the API level (UI-only):

- Health consent — out of scope for Legal V2 (see Security Note below)
- Cookie consent — UI preference, not blocking
- DPA notices — UI informational, not login-blocking

---

## Prisma Model (Expected from DB Team)
...
- Indexed on `userId` — fast lookups for gate checks
...
- `501` — DB model not yet available (temporary, until DB team's PR lands)
...
- `404` — User not found
- `429` — Rate limit exceeded
- `501` — DB model not yet available (temporary)
...
1. ✅ Add `LegalAcceptance` model to `schema.prisma`
2. ✅ Create migration: `prisma migrate dev --name add_legal_acceptance`
3. ✅ Update `User` model to include `legalAcceptances` relation (optional for queries)
4. ⚠️ Add `role` or `subscriptionTier` field to `User` model (if not already present)
5. ✅ Run seed (optional): decide if demo users should have acceptances pre-filled

Once DB model lands:
1. Uncomment `legalRoutes` import and registration in `apps/api/src/index.ts`
2. Uncomment Prisma queries in `apps/api/src/routes/legal.ts`
3. Test with Frontend team
4. Deploy

---

## Testing Checklist (Post-DB Model)
...
- **2026-09-13**: Legal V2 API code ready, blocked on DB `LegalAcceptance` model
- **Next**: DB team adds model + migration → uncomment routes → test → deploy
